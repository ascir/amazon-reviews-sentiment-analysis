...
async function scrapeAmazonProductReviews(productUrl) {
  ...
  const reviewUrl =
    shortUrl.replace("/dp/", "/product-reviews/") +
  ...
  const productDetails = await page.evaluate(() => {
    const title = document.querySelector("#productTitle")
      ...
      : null;
    const imageUrl = document.querySelector("#imgTagWrapperId img")
  ...
  while (true) {
    ...
    const reviews = await page.evaluate(() => {
      return Array.from(document.querySelectorAll(".review-text-content")).map(
        (review) => review.innerText.trim()
      );
    });
  ...
  const sortedPositiveWords = Object.entries(positiveWordFrequency)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5)
    .map((item) => item[0]);
  const sortedNegativeWords = Object.entries(negativeWordFrequency)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5)
    .map((item) => item[0]);
